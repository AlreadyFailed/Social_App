			...
			<swiper class="body_swiper" :current="index" :duration="300">
				<swiper-item v-for="(item, idx) in maxPerson" :key="idx">
					<person :nickname="user_Data[idx].nickname" :age="user_Data[idx].age"
						:gender="user_Data[idx].gender" :famale="src.famale" :male="src.male"
			...
			<view class="screen_body" @tap.stop="testFn">
				...
				<view class="screen_body_gender">
					...
					<view class="gender_sexy" >
						<view class="gender_sexy_pos">
							<view :class="['gender_sexy_img', ,{'main-bg-color main-border-color' : show.gender1}]" :index="flag.gender" @tap="gender">
						...
						<view class="gender_sexy_pos">
							<view :class="['gender_sexy_img', {'main-bg-color main-border-color' : show.gender2}]" @tap="gender">
						...
						<view class="gender_sexy_pos">
							<view :class="['gender_sexy_img', {'main-bg-color main-border-color' : show.gender3}]" @tap="gender">
				...
				<view class="screen_body_age">
					...
					<view class="age_interval">
						...
						<view class="age_interval_dot main-bg-color" 
						:style="{'left': coordinate.x + 'upx'}"
						 @touchmove="drawMove">
				...
				<view class="screen_body_distance">
					...
					<view class="distance_interval">
						<view class="distance_interval_line">
							<view class="distance_interval_line2 main-bg-color" :style="{'width': coordinate.x3 + 'px'}"></view>
						</view>
						<view class="distance_interval_dot main-bg-color"  :style="{'left': coordinate.x3 + 'px'}" @touchmove="drawMove3">
				...
				<view class="screen_body_salary">
					...
					<view class="salary_interval_labels">
						<view ref="salary_box" :class="['labels', 'fs-28', 'text-center', 'color-fff',{'main-bg-color': salary_idx == 0 }]" @tap="salaryInterval($event, 0)">{{salary[0]}}</view>
						<view ref="salary_box" :class="['labels', 'fs-28', 'text-center', 'color-fff',{'main-bg-color': salary_idx == 1 }]" @tap="salaryInterval($event, 1)">{{salary[1]}}</view>
						<view ref="salary_box" :class="['labels', 'fs-28', 'text-center', 'color-fff',{'main-bg-color': salary_idx == 2 }]" @tap="salaryInterval($event, 2)">{{salary[2]}}</view>
						<view ref="salary_box" :class="['labels', 'fs-28', 'text-center', 'color-fff',{'main-bg-color': salary_idx == 3 }]" @tap="salaryInterval($event, 3)">{{salary[3]}}</view>
					</view>
				</view>
				
				<!-- Screen Button -->
				<view @tap="handleScreen"
				class="screen_body_button main-bg-color color-fff text-center fs-600 fs-40">
					搜索
				</view>
			</view>
		</view>
...
<script>
	...
	import {norData} from "../../static/js/index.js";
	import {getDistance} from "../../static/js/calDistance.js"
	export default {
		data() {
			return {
				show: {
